...
public class Aruco {
    ...
    public static List<Marker> detectVispickMarkers(Mat image) {
        Mat ids = new Mat();
        List<Mat> corners = new ArrayList<>();
        Mat placeholder = image.clone();
        Imgproc.cvtColor(placeholder, placeholder, Imgproc.COLOR_RGBA2GRAY);

        detectMarkers(placeholder, Dictionary.get(Aruco.DICT_5X5_1000), corners, ids);

        List<Marker> markers = parseMarkers(corners, ids);
    ...
    private static List<Marker> parseMarkers(List<Mat> corners, Mat ids) {
        List<Marker> markers = new ArrayList<Marker>();
        for (int i = 0; i < corners.size(); i++) {
            if (ids.get(i, 0) != null) {
                markers.add(new Marker((int) ids.get(i, 0)[0], Region.fromCorners(corners.get(i))));
            }
        }
        return clusterMarkers(markers);
    }

    private static List<Marker> clusterMarkers(List<Marker> markers) {
        List<Marker> clustered = new ArrayList<>();
        HashSet<Integer> blacklist = new HashSet<>();

        for (int i = 0; i < markers.size(); i++) {
            if (!blacklist.contains(i)) {
                Region clusteringRegion = clusteringRegion(markers.get(i).getRegion());
                for (int j = 0; j < markers.size(); j++) {
                    if (!blacklist.contains(j) && i != j && clusteringRegion.contains(markers.get(j).getRegion().center())) { // HashSet.contain is O(1)
                        blacklist.add(i);
                        blacklist.add(j);
                        int id = (markers.get(j).getId() << 10) | markers.get(i).getId();
                        Region newRegion = new Region(
                                markers.get(i).getRegion().getTr(),
                                markers.get(j).getRegion().getTl(),
                                markers.get(i).getRegion().getBl(),
                                markers.get(j).getRegion().getBr()
                        );
                        clustered.add(new Marker(id, newRegion));
                    }
                }
            }
        }
    ...
    private static Region clusteringRegion(Region region) {
        return new Region(region.getTl(), pointAlongLine(region.getTl(), region.getTr(), 2.11), region.getBl(), pointAlongLine(region.getBl(), region.getBr(), 2.11));
    }

    private static Point pointAlongLine(Point p1, Point p2, Double distance) {
        return new Point(
                (1 - distance) * p1.x + distance * p2.x,
                (1 - distance) * p1.y + distance * p2.y);
    }
